...
const polygonsToVertexArray = (polygons: Polygon[]): Float32Array => {
	const result: number[] = [];
...
export const generateMesh = async (project: RenderableProject): Promise<MeshInfo> =>
	new Promise((resolve) => {
		setTimeout(() => {
			...
			resolve({
				vertexArray: polygonsToVertexArray(CSG.fromMesh(mesh).toPolygons()),
				dimensions: {
